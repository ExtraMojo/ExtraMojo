...
- Decimal float parsing directly from byte spans: `extramojo.bstr.parse.parse_decimal`
- BBHash
- BitVec
...
### Parsing decimal byte strings

```mojo
from extramojo.bstr.parse import parse_decimal

var value = parse_decimal("-12.5e2".as_bytes())
```

The parser accepts signed decimal numbers and `e`/`E` exponents without allocating
strings. It validates the entire input. The complete significand must fit in
`UInt64`, and the exponent magnitude must be at most `2147483647`. Whitespace,
hex floats, underscores, NaN/Inf spellings, and longer significands are rejected.
Floating-point overflow and underflow produce signed infinity and zero.
...
- The decimal byte parser adapts digit scanning and rounding checks from Zig (MIT;
