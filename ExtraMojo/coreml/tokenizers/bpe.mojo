...
struct TokPair(Copyable, EqualityComparable, Hashable, Movable, Writable):
    var a: UInt16
    var b: UInt16

    @always_inline
    fn __hash__(read self) -> UInt:
        return hash(UnsafePointer(to=self).bitcast[UInt32]()[0])
...
fn count_tokens(read tokens: Span[UInt16]) -> Dict[TokPair, UInt]:
    var counter: Dict[TokPair, UInt] = {}

    for i in range(0, saturating_sub(UInt64(len(tokens)), 1)):
        # TODO: check that we aren't getting bounds checked here
        # TODO: iter over span instead?
        var bpe = TokPair(tokens[i], tokens[i + 1])
        counter.setdefault(bpe, 0) += 1
    return counter


fn max_pair(read counter: Dict[TokPair, UInt]) raises -> TokPair:
    if len(counter) == 0:
        raise "counter must have at least one value"
    var best = TokPair(0, 0)
    var count = 0
...
fn merge(
    best: TokPair,
    next_id: UInt16,
    mut tokens: List[UInt16],
    mut scratch: List[UInt16],
):
    scratch.clear()
    var i = 0
    while i < len(tokens) - 1:
        var tokpair = TokPair(tokens[i], tokens[i + 1])
        if tokpair == best:
            scratch.append(next_id)
            i += 2
        else:
...
fn bpe(read text: String) raises:
    """TODO: creates the 'model', return it"""
    var tokens = [UInt16(b) for b in text.as_bytes()]
    var counter = count_tokens(tokens)

    var vocab_size = 276
    var num_merges = vocab_size - 256
    ids = tokens.copy()

    # here: https://youtu.be/zduSFxRajkE?t=2098&si=TAv-xfrT9f9qBWb3
    merges: Dict[TokPair, UInt16] = {}
    var curr_id = UInt16(UInt8.MAX) + 1
    var scratch = List[UInt16](capacity=len(tokens))
    for i in range(0, num_merges):
        stats = count_tokens(ids)
        # Find the byte pair with highest count
        var best = max_pair(stats)

        merge(best, curr_id, ids, scratch)
        merges[best] = curr_id
        curr_id += 1


fn decode():
    """https://youtu.be/zduSFxRajkE?si=zSM3kT5w39gTIIy7&t=2620"""
    pass
...
def main():
    ...
    #    " into the hearts of programmers worldwide. We all know we ought to"
    #    " “support Unicode” in our software (whatever that means—like using"
    #    " wchar_t for all the strings, right?). But Unicode can be abstruse,"
    #    " and diving into the thousand-page Unicode Standard plus its dozens of"
    ...
    for i in range(0, num_merges):
        ...
        print("merging", best, "into a new token id", curr_id)
        merge(best, curr_id, ids, scratch)
        merges[best] = curr_id
        curr_id += 1
